...
<head>
    ...
    <!-- User role meta tag — used by spa-router.js when serving from views/shared/ -->
    <?php
    $normalizedRole = $_SESSION['user_role'] ?? 'lupto';
    if ($normalizedRole === 'pitco') $normalizedRole = 'picto';
    if (str_ends_with($normalizedRole, '_mto') || $normalizedRole === 'municipal') $normalizedRole = 'municipal';
    ?>
    <meta name="user-role" content="<?= htmlspecialchars($normalizedRole) ?>">
</head>

<body data-role="<?= htmlspecialchars($normalizedRole) ?>">
