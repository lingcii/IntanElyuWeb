/**
 * LUPTO User Management API
 * Role: lupto (can only ADD municipal users)
...
(function () {
if (window.__LUPTO_UM_LOADED__) return;
window.__LUPTO_UM_LOADED__ = true;

function getUmApi() {
    const role = (window.userRole || document.body?.dataset?.role || '').toLowerCase();
    const path = (window.location.pathname || '').toUpperCase();
    let base = window.API_CONFIG?.LUPTO || 'http://localhost:8000/api/lupto';
    if (role === 'picto' || role === 'pitco' || path.includes('/PICTO/')) {
        base = window.API_CONFIG?.PITCO || 'http://localhost:8000/api/pitco';
    }
    return base + '/users';
}

const CACHE_TTL = 60000;
window.__LUPTO_UM_CACHE__ = window.__LUPTO_UM_CACHE__ || { users: null, total: 0, stats: null, municipalities: null, timestamp: 0 };

// Map action strings to Laravel REST endpoints
function umActionToUrl(action, params = {}) {
    const umApi = getUmApi();
    const map = {
        'get_users': `${umApi}`,
        'get_user': `${umApi}/${params.id || ''}`,
        'get_municipalities': (window.API_CONFIG?.BASE_URL || 'http://localhost:8000') + '/api/municipalities',
        'get_audit_logs': `${umApi}/audit-logs`,
        'add_user': `${umApi}`,
        'edit_user': `${umApi}/${params.id || ''}`,
        'toggle_status': `${umApi}/${params.id || ''}/status`,
        'reset_password': `${umApi}/${params.id || ''}/password`,
        'archive_user': `${umApi}/${params.id || ''}/archive`,
        'restore_user': `${umApi}/${params.id || ''}/restore`,
        'delete_user': `${umApi}/${params.id || ''}`,
    };
    const base = map[action] || `${umApi}/${action.replace(/_/g, '-')}`;
...
document.addEventListener('DOMContentLoaded', () => {
    const cache = window.__LUPTO_UM_CACHE__;
    ...
    if (_initialBoot && cache.users) {
        ...
        // Background auto-refresh every 60 seconds (silent, no spinner)
...
async function refreshTable() {
    ...
    _currentPage = 1;
    window.__LUPTO_UM_CACHE__.timestamp = 0;
    await loadUsers();
...
async function loadUsers(silent = false) {
    ...
    // Only show loading spinner for non-silent (user-initiated) refreshes
    if (!silent) {
    ...
    try {
        ...
        console.error('[LUPTO-UM] loadUsers:', err);
...
// ── ADD USER MODAL (PICTO: LUPTO + Municipal, LUPTO: Municipal only)
function openAddModal() {
    resetForm();
    const role = (window.userRole || '').toLowerCase();
    const isPicto = role === 'picto' || role === 'pitco';

    setText('formModalTitleText', isPicto ? 'Add User Account' : 'Add Municipal User');
    setText('formSubmitText', 'Create User');

    const btnLabel = document.getElementById('btnAddUserLabel');
    if (btnLabel) btnLabel.textContent = isPicto ? 'Add User' : 'Add Municipal User';
...
async function openEditModal(id) {
    ...
    try {
        ...
        const isPicto = userRoleEnv === 'picto' || userRoleEnv === 'pitco';

        if (roleSelect) {
            const rawRole = (u.role === 'pitco' ? 'picto' : (u.role.endsWith('_mto') ? 'municipal' : u.role));
            if (isPicto) {
                roleSelect.innerHTML = `
                    <option value="municipal">Municipal Tourism Office (MTO)</option>
                    <option value="lupto">La Union Provincial Tourism Office (LUPTO)</option>
                    ${rawRole === 'picto' ? '<option value="picto">PICTO Admin</option>' : ''}
                `;
                roleSelect.disabled = false;
            } else {
                roleSelect.innerHTML = `
                    <option value="municipal">Municipal Tourism Office (MTO)</option>
                    ${rawRole === 'lupto' ? '<option value="lupto">LUPTO</option>' : ''}
                `;
                roleSelect.disabled = true;
            }
...
function onRoleChange() {
    ...
    if (isMunicipal) {
        ...
        const muniSelect = document.getElementById('formMunicipality');
...
async function submitUserForm() {
    ...
    if (isMunicipal && muniId) body.municipality_id = parseInt(muniId, 10);
    ...
    document.getElementById('umSaveConfirmText').textContent = isEdit
        ...
        : `Are you sure you want to add "${name}" as a ${role.toUpperCase()} user?`;
...
function resetForm() {
    ...
    const roleHidden = document.getElementById('formRole');
    const roleDisplay = document.getElementById('formRoleDisplay');
    if (roleHidden) { roleHidden.value = 'municipal'; }
    if (roleDisplay) { roleDisplay.value = 'MUNICIPAL'; }
...
function confirmArchiveUser(id, name) {
    ...
    document.getElementById('confirmText').innerHTML =
        `Are you sure you want to <strong>archive</strong> the account of <strong>${escHtml(name)}</strong>?`;
...
function confirmRestoreUser(id, name) {
    ...
    document.getElementById('confirmText').innerHTML =
        `Are you sure you want to <strong>restore</strong> the account of <strong>${escHtml(name)}</strong>?`;
...
function confirmDeleteUser(id, name) {
    ...
    document.getElementById('confirmText').innerHTML =
        `Are you sure you want to <strong>permanently delete</strong> the account of <strong>${escHtml(name)}</strong>? This action cannot be undone.`;
