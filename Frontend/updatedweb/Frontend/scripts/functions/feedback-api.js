(function () {
    ...
    async function apiFetch(url, useCache = true) {
        ...
        if (window.API_CONFIG && typeof window.API_CONFIG.get === 'function') {
            data = await window.API_CONFIG.get(url);
        } else {
    ...
    async function loadDashboardStats(silent = false) {
        ...
        try {
            const d = await apiFetch(`${getBaseUrl()}/feedback/dashboard-stats`);
    ...
    function renderCharts(tab, d) {
        ...
        if (trendCanvas && window.Chart && d.monthly_trend?.length) {
            ...
            charts.trend = new Chart(trendCanvas, {
                type: 'line',
                data: {
                    labels: trend.map(t => t.month), datasets: [
    ...
    async function loadGallery(page = 1, silent = false) {
        ...
        try {
            const d = await apiFetch(`${getBaseUrl()}/feedback/gallery?${params}`);
    ...
    async function loadTable(page = 1) {
        ...
        try {
            const d = await apiFetch(`${getBaseUrl()}/feedback/table?${params}`);
            renderTable(tbody, d);
        } catch (e) {
    ...
    async function openSpotDetail(spotId) {
        ...
        try {
            const d = await apiFetch(`${getBaseUrl()}/feedback/spot-details/${spotId}?per_page=10&sort=${detailSort}`);
            renderModal(d);
        } catch (e) {
    ...
    async function loadMoreReviews(page) {
        ...
        try {
            const d = await apiFetch(`${getBaseUrl()}/feedback/spot-details/${detailSpotId}?per_page=10&page=${page}&sort=${detailSort}`);
