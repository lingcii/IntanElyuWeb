(function () {
    ...
    if (window.L && window.L.DomUtil && !window.L.DomUtil.isPatched) {
        try {
            var origGetPos = window.L.DomUtil.getPosition;
            if (typeof origGetPos === 'function') {
                window.L.DomUtil.getPosition = function (el) {
                    if (!el) {
                        var P = window.L.Point || function (x, y) { this.x = x || 0; this.y = y || 0; };
                        return new P(0, 0);
                    }
                    return origGetPos.call(window.L.DomUtil, el);
                };
            }
            var origSetPos = window.L.DomUtil.setPosition;
            if (typeof origSetPos === 'function') {
                window.L.DomUtil.setPosition = function (el, pt) {
                    if (!el) return;
                    return origSetPos.call(window.L.DomUtil, el, pt);
                };
            }
