/**
 * spa-router.js
 * Single Page Application (SPA) Router for views.
 * 
 * Intercepts navigation links to dynamically fetch, load, and switch view tab contents 
 * without full browser page reloads. Manages scroll positions, form input states, 
 * Leaflet map sizing, script re-execution, and browser history (pushState/popstate).
 */

(function () {
    'use strict';

    // Check if we are in the views directory
    const path = window.location.pathname;
    if (!path.includes('/views/')) {
        return; // Only run inside views directory
    }

    function getStorageRole() {
        const bodyRole = document.body?.dataset?.role || '';
        if (bodyRole) return bodyRole;
        return document.querySelector('meta[name="user-role"]')?.content || 'user';
    }
    ...
    // Key prefix for sessionStorage persistence — resolved after DOM is ready
    let STORAGE_PREFIX = `spa_state_${getStorageRole()}_`;
    ...
    // Safely patch Leaflet utilities and L.map to capture map instances and prevent undefined DOM element errors
    function patchLeaflet() {
        if (!window.L) return;

        if (window.L.DomUtil && !window.L.DomUtil.isPatched) {
            ...
            if (typeof origGetPosition === 'function') {
                window.L.DomUtil.getPosition = function (el) {
                    if (!el) {
                        const PointClass = window.L.Point || function (x, y) { this.x = x || 0; this.y = y || 0; };
        ...
        if (window.L.map && !window.L.map.isPatched) {
            const originalLMap = window.L.map;
            window.L.map = function (id, options) {
                const mapInstance = originalLMap.call(window.L, id, options);
            ...
            window.L.map.isPatched = true;
        }
    }

    // Call once at startup and setup a setter trap in case Leaflet loads asynchronously
    patchLeaflet();
