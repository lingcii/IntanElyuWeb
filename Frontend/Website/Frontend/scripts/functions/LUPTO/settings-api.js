...
(function(){
    ...
    const API  = BASE + '/api/lupto/settings/backup';
    ...
    async function loadLogs() {
        ...
        try {
            const d = await apiFetch('GET', BASE + '/api/lupto/activity-logs?per_page=10&module=Backup');
        ...
        confirmRestore: async function() {
            ...
            try {
                ...
                if (window._lupto_uploadFile) {
                    ...
                    d = await apiFetch('POST', API + '/restore', fd, true);
                } else {
                    d = await apiFetch('POST', API + '/restore', { filename: _restoreTarget });
                    _restoreTarget = null;
                }
                if (d.success) {
