...
const UM_API = window.API_CONFIG?.LUPTO + '/users' || 'http://localhost:8000/api/lupto/users';
...
// ── ADD USER MODAL (Municipal only)
function openAddModal() {
    resetForm();
    setText('formModalTitleText', 'Add Municipal User');
    setText('formSubmitText', 'Create User');

    // Lock role to Municipal only
    ...
    // Auto-show municipality selector since role is municipal
    onRoleChange();
...
async function openEditModal(id) {
    ...
    try {
        ...
        // In edit mode, show the current role in the display field and hidden field
        const roleHidden = document.getElementById('formRole');
        const roleDisplay = document.getElementById('formRoleDisplay');
        if (roleHidden) roleHidden.value = u.role;
        if (roleDisplay) roleDisplay.value = u.role.toUpperCase();
...
async function submitUserForm() {
    ...
    // For edit mode, get the role from the disabled select
