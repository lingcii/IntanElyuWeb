...
function showInvalidLocationModal(muniName, customMessage = null) {
    ...
    if (p) {
        if (customMessage) {
            ...
            p.innerHTML = `The selected location is outside the boundary of the chosen municipality.<br/><br/>Please place the pin within the official boundary of <strong id="invalidLocationMuniName">${muniName}</strong>.`;
        }
    }
    ...
    try {
        ...
        document.getElementById('modalBody').innerHTML = `
            ...
            <div style="display:flex;gap:8px;align-items:center;margin-bottom:16px;flex-wrap:wrap;">
                ...
                ` : ''}
                ${(spot.status && spot.status !== 'approved') ? `
                    <span style="font-size:13px;font-weight:700;padding:4px 12px;border-radius:20px;background:${spot.status === 'approved' ? '#10B981' : spot.status === 'pending' ? '#F59E0B' : '#DC2626'};color:#FFFFFF;">${spot.status === 'approved' ? 'Approved' : spot.status === 'pending' ? 'Pending' : 'Rejected'}</span>
                ` : ''}
                ${spot.is_maintenance ? `
            ...
            `<span style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:12px;font-weight:600;background:#DBEAFE;color:#2563EB;">${escapeHtml(c)}</span>`
                ...
                <div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:8px;padding:12px;grid-column:1/-1;">
                    <div style="font-size:11px;color:#1D4ED8;font-weight:700;text-transform:uppercase;margin-bottom:4px;"><i class="fas fa-user-plus"></i> Added Tourist Spots By</div>
                    <div style="font-size:14px;font-weight:700;color:#1D4ED8;">
                        ${(spot.creator_role || '').toUpperCase() || 'LUPTO'}
                        <span style="font-weight:400;color:#3B82F6;margin-left:6px;">(${escapeHtml(spot.creator?.name || 'N/A')})</span>
                    </div>
                </div>
                ` : ''}
            </div>
