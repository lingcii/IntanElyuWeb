(function () {
    ...
    function populateFilterDropdowns() {
        ...
        if (muniSel && muniSel.options.length <= 1 && galleryData.municipalities.length) {
            galleryData.municipalities.forEach(m => muniSel.appendChild(new Option(m.name, m.id)));
        }
        const catSel = document.getElementById('fb-filter-category');
        if (catSel && catSel.options.length <= 1 && galleryData.categories.length) {
            galleryData.categories.forEach(c => catSel.appendChild(new Option(c, c)));
        }
    }
    ...
    async function openSpotDetail(spotId) {
        ...
        if (!overlay) {
            ...
            overlay.innerHTML = `<div class="fb-modal" id="fb-modal-inner" role="dialog" aria-modal="true"><div class="fb-modal-header"><h2 class="fb-modal-header-title"><i class="fas fa-comments"></i> Tourist Spot Feedback</h2><button class="fb-modal-close" onclick="window.closeFeedbackModal()" aria-label="Close"><i class="fas fa-times"></i></button></div><div class="fb-modal-body" id="fb-modal-body"><div class="fb-empty-state"><i class="fas fa-spinner fa-spin"></i><p>Loading…</p></div></div></div>`;
    ...
    function renderModal(d) {
        ...
        document.getElementById('fb-modal-body').innerHTML = `<div class="fb-modal-spot-hero">${sliderHtml}<div class="fb-modal-spot-info"><h2 class="fb-modal-spot-name">${escHtml(spot.name)}</h2><div class="fb-modal-spot-meta"><span class="fb-meta-badge muni"><i class="fas fa-location-dot"></i> ${escHtml(spot.municipality||'Unknown')}</span><span class="fb-meta-badge cat"><i class="fas fa-tag"></i> ${escHtml(spot.category||'Spot')}</span></div><div class="fb-modal-big-rating"><span class="fb-modal-big-num">${(spot.avg_rating||0).toFixed(1)}</span><div><div class="fb-modal-big-stars">${[1,2,3,4,5].map(i=>`<i class="fas fa-star fb-modal-big-star${i>Math.round(spot.avg_rating||0)?' empty':''}"></i>`).join('')}</div><div class="fb-modal-review-count">${(spot.total_reviews||0).toLocaleString()} reviews</div></div></div><ul class="fb-breakdown-list">${breakdownHtml}</ul></div></div>${spot.description?`<p style="font-size:13px;color:#475569;margin:0 0 18px;line-height:1.7;">${escHtml(spot.description)}</p>`:''}<div class="fb-reviews-header"><h3 class="fb-reviews-title"><i class="fas fa-list-ul"></i> All Reviews</h3><select id="fb-modal-sort" class="fb-filter-select" style="min-width:130px;font-size:12px;padding:6px 10px;"><option value="newest">Newest First</option><option value="oldest">Oldest First</option><option value="highest_rated">Highest Rated</option><option value="lowest_rated">Lowest Rated</option></select></div><div id="fb-modal-reviews-list">${renderReviews(d.reviews)}</div><div id="fb-modal-pagination" style="margin-top:12px;"></div>`;
        renderPagination('fb-modal-pagination', d.current_page, d.last_page, loadMoreReviews);
