(function () {
    ...
    if (window.__municipalFeedbackLoaded) {
        if (typeof window.initFeedbackModule === 'function') window.initFeedbackModule();
        return;
    }
    window.__municipalFeedbackLoaded = true;

    const BASE = window.API_CONFIG?.MUNICIPAL || 'http://127.0.0.1:8000/api/municipal';
    ...
    async function loadDashboardStats(silent=false) {
        ...
        try {
            ...
            set('spots_reviewed',(s.spots_reviewed||0).toLocaleString());
            set('total_feedback',(s.total_feedback||0).toLocaleString());
            set('avg_rating',(s.avg_rating||0).toFixed(1));
            set('five_star',(s.five_star||0).toLocaleString());
            set('four_star',(s.four_star||0).toLocaleString());
            set('three_star',(s.three_star||0).toLocaleString());
            set('two_star',(s.two_star||0).toLocaleString());
            set('one_star',(s.one_star||0).toLocaleString());
            renderCharts(tab, d);
        } catch(e) {
            if (!silent) {
                const set=(k,v)=>{const el=tab.querySelector(`[data-fb-kpi="${k}"]`);if(el)el.textContent=v;};
                set('spots_reviewed', '0'); set('total_feedback', '0'); set('avg_rating', '0.0');
                set('five_star', '0'); set('four_star', '0'); set('three_star', '0'); set('two_star', '0'); set('one_star', '0');
            }
        }
    }
    ...
    function renderCharts(tab, d) {
        ...
        if (distCanvas&&window.Chart) {
            ...
            charts.dist=new Chart(distCanvas,{type:'doughnut',data:{labels:['5 ★','4 ★','3 ★','2 ★','1 ★'],datasets:[{data:[br[5]||0,br[4]||0,br[3]||0,br[2]||0,br[1]||0],backgroundColor:['#059669','#0B5394','#F59E0B','#EA580C','#DC2626'],borderWidth:2,borderColor:'#fff'}]},options:{cutout:'65%',responsive:true,maintainAspectRatio:true,plugins:{legend:{position:'right',labels:{font:{size:12},padding:12}}}}});
        }
        destroyChart('trend');
        ...
        if (trendCanvas&&window.Chart&&d.monthly_trend?.length) {
            ...
            charts.trend=new Chart(trendCanvas,{type:'line',data:{labels:trend.map(t=>t.month),datasets:[{label:'Reviews',data:trend.map(t=>t.count),borderColor:'#c2410c',backgroundColor:'rgba(194,65,12,0.08)',fill:true,tension:0.4,pointRadius:4,yAxisID:'y'},{label:'Avg Rating',data:trend.map(t=>t.avg_rating),borderColor:'#D97706',backgroundColor:'transparent',tension:0.4,pointRadius:4,borderDash:[5,4],yAxisID:'y1'}]},options:{responsive:true,maintainAspectRatio:false,scales:{y:{position:'left',title:{display:true,text:'Reviews',font:{size:11}},grid:{color:'#f1f5f9'},ticks:{font:{size:11}}},y1:{position:'right',title:{display:true,text:'Avg ★',font:{size:11}},min:0,max:5,grid:{display:false},ticks:{font:{size:11}}},x:{grid:{display:false},ticks:{font:{size:11}}}},plugins:{legend:{labels:{font:{size:12}}}}}});
        }
    ...
    function renderTopList(tab,selector,items,metric){
        const el=tab.querySelector(selector);if(!el)return;
    ...
    async function loadGallery(page=1, silent=false){
        galleryPage=page;
        const grid=document.getElementById('fb-gallery-grid');if(!grid)return;
    ...
    function renderGallery(grid,d){
        if(!d.data?.length){grid.innerHTML=`<div class="fb-empty-state" style="grid-column:1/-1;"><i class="fas fa-comments"></i><h3>No Feedback Found</h3><p>Your municipality's tourist spots have no reviews yet.</p></div>`;renderPagination('fb-gallery-pagination',0,0,loadGallery);return;}
        grid.innerHTML=d.data.map(spot=>{const img=imgUrl(spot.photo_url);const imgTag=img?`<img src="${escHtml(img)}" alt="${escHtml(spot.name)}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=\\'fb-spot-img-placeholder\\'><i class=\\'fas fa-mountain-sun\\'></i></div>';">`:` <div class="fb-spot-img-placeholder"><i class="fas fa-mountain-sun"></i></div>`;return`<div class="fb-spot-card" onclick="window.openFeedbackSpotDetail(${spot.id})" role="button" tabindex="0"><div class="fb-spot-img-wrap">${imgTag}<span class="fb-spot-category-badge">${escHtml(spot.category||'Spot')}</span></div><div class="fb-spot-card-body"><h3 class="fb-spot-name">${escHtml(spot.name)}</h3><div class="fb-spot-municipality"><i class="fas fa-location-dot"></i> ${escHtml(spot.municipality||'')}</div><div class="fb-spot-rating-row"><div>${renderStars(spot.avg_rating)}<span class="fb-avg-rating" style="margin-left:6px;">${(spot.avg_rating||0).toFixed(1)}</span></div><span class="fb-review-count">${(spot.total_reviews||0).toLocaleString()} reviews</span></div></div><div class="fb-spot-card-footer"><i class="fas fa-eye"></i> View Feedback</div></div>`;}).join('');
        renderPagination('fb-gallery-pagination',d.current_page,d.last_page,loadGallery);
    }

    function getTableFilters(){return{search:document.getElementById('fb-search-input')?.value.trim()||'',category:document.getElementById('fb-filter-category')?.value||'',rating:document.getElementById('fb-filter-rating')?.value||'',sort:document.getElementById('fb-sort-select')?.value||'newest',date_from:document.getElementById('fb-date-from')?.value||'',date_to:document.getElementById('fb-date-to')?.value||''};}

    async function loadTable(page=1){
        tablePage=page;
        const tbody=document.getElementById('fb-table-body');if(!tbody)return;
        ...
        try{const d=await apiFetch(`${BASE}/feedback/table?${params}`);renderTable(tbody,d);}
        catch(e){tbody.innerHTML=`<tr><td colspan="7" style="text-align:center;padding:30px;color:#EF4444;"><i class="fas fa-exclamation-circle"></i> Failed to load.</td></tr>`;}
    }

    function renderTable(tbody,d){
        if(!d.data?.length){tbody.innerHTML=`<tr><td colspan="7"><div class="fb-empty-state"><i class="fas fa-comments"></i><h3>No Reviews Found</h3><p>Try adjusting your filters.</p></div></td></tr>`;renderPagination('fb-table-pagination',0,0,loadTable);return;}
        tbody.innerHTML=d.data.map(row=>`<tr><td><span class="fb-table-spot-name">${escHtml(row.tourist_spot)}</span></td><td><span style="font-size:12px;">${escHtml(row.municipality)}</span></td><td>${ratingBadge(row.rating)}</td><td><span class="fb-table-comment">${escHtml(row.comment||'—')}</span></td><td><div style="display:flex;align-items:center;gap:6px;">${avatarHtml(row.user_name,row.user_avatar,28)}<span>${escHtml(row.user_name)}</span></div></td><td style="font-size:12px;color:#64748b;white-space:nowrap;">${escHtml(row.date)}</td><td><button class="fb-page-btn" onclick="window.openFeedbackSpotDetail(null,${row.id})" style="font-size:11px;padding:4px 10px;height:28px;"><i class="fas fa-eye"></i></button></td></tr>`).join('');
    ...
    async function openSpotDetail(spotId){
        ...
        catch(e){document.getElementById('fb-modal-body').innerHTML=`<div class="fb-empty-state"><i class="fas fa-exclamation-circle"></i><h3>Error</h3><p>Could not load spot details.</p></div>`;}
    }

    async function loadMoreReviews(page){
        detailPage=page;
        const container=document.getElementById('fb-modal-reviews-list');if(!container)return;
        container.innerHTML=`<div class="fb-empty-state"><i class="fas fa-spinner fa-spin"></i><p>Loading…</p></div>`;
        try{const d=await apiFetch(`${BASE}/feedback/spot-details/${detailSpotId}?per_page=10&page=${page}&sort=${detailSort}`);container.innerHTML=renderReviews(d.reviews);renderPagination('fb-modal-pagination',d.current_page,d.last_page,loadMoreReviews);}
        catch(e){}
    }
    ...
    function switchView(view){
        ...
        if(gs)gs.style.display=view==='gallery'?'':'none';if(ts)ts.style.display=view==='table'?'':'none';
        if(bg)bg.classList.toggle('active',view==='gallery');if(bt)bt.classList.toggle('active',view==='table');
        if(df)df.style.display=view==='table'?'':'none';
        if(view==='gallery')loadGallery(1);else loadTable(1);
    }

    function onSearchInput(){clearTimeout(searchTimeout);searchTimeout=setTimeout(()=>{if(currentView==='gallery')loadGallery(1);else loadTable(1);},350);}

    function openLightbox(url){
        let lb=document.getElementById('fb-lightbox');
        if(!lb){lb=document.createElement('div');lb.id='fb-lightbox';lb.className='fb-lightbox-overlay';lb.innerHTML=`<img class="fb-lightbox-img" src="" alt="Review photo">`;lb.addEventListener('click',()=>lb.style.display='none');lb.querySelector('.fb-lightbox-img').addEventListener('click',e=>e.stopPropagation());document.body.appendChild(lb);}
        lb.querySelector('.fb-lightbox-img').src=url;lb.style.display='flex';
    }
    ...
    window.setSpotSlide=function(index){if(!window._currentSpotGallery||!window._currentSpotGallery.length)return;index=(index+window._currentSpotGallery.length)%window._currentSpotGallery.length;window._currentSpotSlideIndex=index;const imgEl=document.getElementById('fb-slider-img');const counterEl=document.getElementById('fb-slider-counter');if(imgEl)imgEl.src=window._currentSpotGallery[index];if(counterEl)counterEl.textContent=(index+1)+' / '+window._currentSpotGallery.length;document.querySelectorAll('.fb-slider-thumb').forEach((t,i)=>t.classList.toggle('active',i===index));};
    window.changeSpotSlide=function(dir){window.setSpotSlide((window._currentSpotSlideIndex||0)+dir);};
    ...
    function init(force=false){
        ...
        if (!isInitialized) {
            const si=tab.querySelector('#fb-search-input');if(si)si.addEventListener('input',onSearchInput);
            ['fb-filter-category','fb-filter-rating','fb-sort-select'].forEach(id=>{const el=tab.querySelector(`#${id}`);if(el)el.addEventListener('change',()=>{if(currentView==='gallery')loadGallery(1);else loadTable(1);});});
            ['fb-date-from','fb-date-to'].forEach(id=>{const el=tab.querySelector(`#${id}`);if(el)el.addEventListener('change',()=>{if(currentView==='table')loadTable(1);});});
        }
