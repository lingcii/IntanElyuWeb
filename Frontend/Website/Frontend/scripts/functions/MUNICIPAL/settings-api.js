/* ============================================================
   MUNICIPAL Settings Page — External JS
   scripts/functions/MUNICIPAL/settings-api.js
   ============================================================ */

// ── Backup Module (Municipality-Scoped) ───────────────────────
(function(){
    const BASE = (window.API_CONFIG?.BASE_URL || ('http://' + (window.location.hostname || '127.0.0.1') + ':8000'));
    const API  = BASE + '/api/municipal/settings/backup';
    let _rt = null, _dt = null;

    function showLoading(t) {
        const textEl = document.getElementById('mun-backupLoadingText');
        if (textEl) textEl.textContent = t || 'Processing...';
        const overlay = document.getElementById('mun-backupLoadingOverlay');
        if (overlay) overlay.style.display = 'flex';
        ['mun-btnCreate','mun-btnRestore'].forEach(id => { const el=document.getElementById(id); if(el) el.disabled=true; });
    }
    function hideLoading() {
        const overlay = document.getElementById('mun-backupLoadingOverlay');
        if (overlay) overlay.style.display = 'none';
        ['mun-btnCreate','mun-btnRestore'].forEach(id => { const el=document.getElementById(id); if(el) el.disabled=false; });
    }
    function showToast(msg, type) {
        let t = document.getElementById('mun-backupToast');
    ...
    async function loadStats() {
        try {
            ...
            if (d.success) {
                document.getElementById('mun-statTotalVal').textContent = d.total;
                document.getElementById('mun-statLastVal').textContent  = d.last_backup;
                document.getElementById('mun-statDbSizeVal').textContent = d.db_size;
                document.getElementById('mun-statStatusVal').textContent = d.status;
                document.getElementById('mun-statStatusVal').style.color = d.status==='Healthy'?'#16a34a':'#d97706';
            }
        } catch(e){}
    }
    async function loadList() {
        const tbody = document.getElementById('mun-backupTableBody');
        ...
        try {
            ...
            if (!d.success||!d.backups.length) { tbody.innerHTML='<tr><td colspan="4" style="text-align:center;padding:24px;color:#94a3b8;"><i class="fas fa-folder-open" style="font-size:28px;display:block;margin-bottom:8px;"></i>No backups found for your municipality.</td></tr>'; return; }
            tbody.innerHTML = d.backups.map(b=>`
                <tr>
                    ...
                    <td style="text-align:center;"><div style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;">
                        <button onclick="window.mun_backup.download('${b.filename}')" style="background:#eff6ff;color:#3b82f6;border:1px solid #bfdbfe;border-radius:6px;padding:5px 12px;font-size:12px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:5px;"><i class="fas fa-download"></i> Download</button>
                        <button onclick="window.mun_backup.openRestore('${b.filename}')" style="background:#fff7ed;color:#d97706;border:1px solid #fed7aa;border-radius:6px;padding:5px 12px;font-size:12px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:5px;"><i class="fas fa-undo-alt"></i> Restore</button>
                        <button onclick="window.mun_backup.openDelete('${b.filename}')" style="background:#fef2f2;color:#dc2626;border:1px solid #fecaca;border-radius:6px;padding:5px 12px;font-size:12px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:5px;"><i class="fas fa-trash-alt"></i> Delete</button>
                    </div></td>
                </tr>`).join('');
    ...
    async function loadLogs() {
        const c = document.getElementById('mun-backupLogsContainer');
        try {
            const d = await apiFetch('GET', BASE+'/api/municipal/activity-logs?per_page=10&module=Backup');
    ...
    function openRestore(fn) { _rt=fn; document.getElementById('mun-restoreFileName').textContent=fn; document.getElementById('mun-restoreModal').style.display='flex'; }
    function closeRestoreModal() { document.getElementById('mun-restoreModal').style.display='none'; _rt=null; }
    function openDelete(fn)  { _dt=fn; document.getElementById('mun-deleteFileName').textContent=fn; document.getElementById('mun-deleteModal').style.display='flex'; }
    function closeDeleteModal() { document.getElementById('mun-deleteModal').style.display='none'; _dt=null; }
    ...
    async function uploadRestore(input) {
        ...
        _rt=file.name; window._mun_uploadFile=file; input.value='';
        document.getElementById('mun-restoreFileName').textContent=file.name;
        document.getElementById('mun-restoreModal').style.display='flex';
    }
    window.mun_backup = {
        ...
        confirmRestore: async function() {
            ...
            try {
                let d;
                if (window._mun_uploadFile) { const fd=new FormData(); fd.append('backup_file',window._mun_uploadFile); window._mun_uploadFile=null; d=await apiFetch('POST',API+'/restore',fd,true); }
...
window.confirmUpdateSecuritySettings = async function() {
    ...
    var toast = document.getElementById('munSaveToast');
    if (!toast) {
        toast = document.createElement('div');
        toast.id = 'munSaveToast';
