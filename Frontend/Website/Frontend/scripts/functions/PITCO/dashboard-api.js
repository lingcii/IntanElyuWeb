(function () {
    ...
    function setupMapFilters(mapEl, spots) {
        ...
        var isMunicipalRole = window.location.pathname.includes('/MUNICIPAL/');
        ...
        window.MapMarkersConfig.buildFilterControls(filterContainer, function (selectedCats, selectedClasses, selectedMunicipality) {
            var map = mapEl._leaflet_map;
            if (!map) return;

            map._selectedCategories = selectedCats;
            map._selectedClassifications = selectedClasses;
            map._selectedMunicipality = selectedMunicipality || '';

            window.MapMarkersConfig.updateMapSpots(mapEl, spots);
            window.MapMarkersConfig.zoomToMunicipality(map, selectedMunicipality, spots);
            filterDashboardByMunicipality(selectedMunicipality, spots);
        }, { hideMunicipality: isMunicipalRole });
    }

    // ── Municipality KPI / Chart filter (client-side, no server request) ──────────
    function filterDashboardByMunicipality(muniName, allSpots) {
        var container = document.getElementById('spa-tab-dashboard.php') || document;
        var spots = allSpots || (currentDashboardData ? currentDashboardData.touristSpots || [] : []);
        var filtered = muniName
            ? spots.filter(function (s) {
                var spotMuni = (s.municipality_name || (s.municipality && s.municipality.name) || '').trim();
                return spotMuni.toLowerCase() === muniName.toLowerCase();
            })
            : spots;
        ...
        if (elSpots && typeof window.animateKpiValue === 'function') window.animateKpiValue(elSpots, totalSpots);

        // ── Province-wide note on non-filterable KPIs ──
        var provinceWideKpis = ['total-tourist-users', 'total-points-earned', 'total-visits', 'total-fare-matrix'];
        provinceWideKpis.forEach(function (kpiKey) {
            var card = container.querySelector('[data-kpi="' + kpiKey + '"]');
            if (!card) return;
            var existingNote = card.querySelector('.dm-kpi-province-note');
            if (muniName) {
                if (!existingNote) {
                    var note = document.createElement('span');
                    note.className = 'dm-kpi-province-note';
                    note.style.cssText = 'display:block;font-size:10px;color:#94A3B8;margin-top:2px;font-style:italic;';
                    note.textContent = 'Province-wide';
                    var info = card.querySelector('.lupto-kpi-info');
        ...
        if (catCtx && _dashboardCharts.category) {
            var catMap = {};
            filtered.forEach(function (s) {
                String(s.category || '').split(',').forEach(function (part) {
                    var cat = part.trim();
                    if (cat) catMap[cat] = (catMap[cat] || 0) + 1;
                });
            });
            var catData = _dashboardCharts.category.data;
            if (catData && catData.datasets && catData.datasets[0]) {
                catData.datasets[0].data = catData.labels.map(function (label) {
                    return catMap[label] || 0;
                });
                _dashboardCharts.category.update('active');
            }
        }

        // ── Classification Chart ──
        var classCtx = document.getElementById('classificationChart');
        if (classCtx && _dashboardCharts.classification) {
            var counts = { EXIST: 0, EMERGE: 0, POTENTIAL: 0 };
            filtered.forEach(function (s) {
                if (counts[s.classification_status] !== undefined) counts[s.classification_status]++;
            });
            var classData = _dashboardCharts.classification.data;
            if (classData && classData.datasets && classData.datasets[0]) {
                classData.datasets[0].data = [counts.EXIST, counts.EMERGE, counts.POTENTIAL];
                _dashboardCharts.classification.update('active');
            }
        }
    }
