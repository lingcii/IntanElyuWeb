(function () {
    ...
    // ── Guard against re-execution on SPA navigation ─────────────────────────
    if (window.__pitcoFeedbackLoaded) {
        if (typeof window.initFeedbackModule === 'function') {
            window.initFeedbackModule();
        }
        return;
    }
    window.__pitcoFeedbackLoaded = true;

    // ── API base ──────────────────────────────────────────────────────────────
    const BASE = window.API_CONFIG?.PITCO || 'http://127.0.0.1:8000/api/pitco';

    // ── State ─────────────────────────────────────────────────────────────────
    let currentView    = 'gallery';   // 'gallery' | 'table'
    let galleryPage    = 1;
    let tablePage      = 1;
    let detailPage     = 1;
    ...
    function imgUrl(path) {
        ...
        // Feedback images stored in Laravel public storage
    ...
    function renderStars(rating, size = 'sm') {
        ...
        for (let i = 1; i <= 5; i++) {
            html += `<i class="fas fa-star fb-star${i > r ? ' empty' : ''}"></i>`;
        }
        html += '</span>';
        return html;
    }
    ...
    function avatarHtml(name, avatarPath, size = 36) {
        ...
        if (url) return `<img src="${url}" class="fb-reviewer-avatar" style="width:${size}px;height:${size}px;" onerror="this.style.display='none';this.nextSibling.style.display='flex';" alt="${escHtml(name)}"><span class="fb-reviewer-avatar-placeholder" style="width:${size}px;height:${size}px;display:none;">${escHtml((name || 'A')[0].toUpperCase())}</span>`;
    ...
    // ── Helper: skeleton gallery ──────────────────────────────────────────────
    function skeletonGallery(n = 8) {
        return Array.from({ length: n }, () => `
            <div class="fb-skeleton-card">
                <div class="fb-skeleton fb-skeleton-img"></div>
                <div style="padding:14px 16px;">
                    <div class="fb-skeleton fb-skeleton-text w80"></div>
                    <div class="fb-skeleton fb-skeleton-text w40" style="margin-top:10px;"></div>
                    <div class="fb-skeleton fb-skeleton-text w60" style="margin-top:14px;"></div>
                </div>
            </div>`).join('');
    }
    ...
    function renderKpis(tab, stats) {
        const set = (key, val) => {
            const el = tab.querySelector(`[data-fb-kpi="${key}"]`);
    ...
    function renderCharts(tab, d) {
        // 1. Rating Distribution (Doughnut)
        destroyChart('dist');
        ...
        if (distCanvas && window.Chart) {
            ...
            charts.dist = new Chart(distCanvas, {
                ...
                data: {
                    labels: ['5 ★', '4 ★', '3 ★', '2 ★', '1 ★'],
                    datasets: [{
                        data: [breakdown[5]||0, breakdown[4]||0, breakdown[3]||0, breakdown[2]||0, breakdown[1]||0],
                        backgroundColor: ['#059669','#0B5394','#F59E0B','#EA580C','#DC2626'],
                        borderWidth: 2,
                        borderColor: '#fff',
                    }]
                },
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: { callbacks: { label: ctx => ` ${ctx.label}: ${ctx.raw.toLocaleString()} reviews` } }
                    }
                }
            });
        }

        // 2. Municipality Rating Comparison (Horizontal Bar)
        destroyChart('muni');
        ...
        if (muniCanvas && window.Chart && d.municipality_comparison?.length) {
            ...
            charts.muni = new Chart(muniCanvas, {
                type: 'bar',
                data: {
                    labels: muni.map(m => m.name),
                    datasets: [{
                        label: 'Average Rating',
                        data: muni.map(m => m.avg_rating),
                        backgroundColor: muni.map((_, i) =>
                            `hsla(${210 + i * 10}, 70%, ${45 + i * 2}%, 0.85)`),
                        borderRadius: 6,
                        borderSkipped: false,
                    }]
                },
                options: {
                    ...
                    scales: {
                        x: { min: 0, max: 5, ticks: { font: { size: 11 } }, grid: { color: '#f1f5f9' } },
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: ctx => ` ⭐ ${ctx.raw.toFixed(2)} avg rating`
                            }
                        }
                    }
                }
        ...
        if (trendCanvas && window.Chart && d.monthly_trend?.length) {
            ...
            charts.trend = new Chart(trendCanvas, {
                ...
                options: {
                    ...
                    maintainAspectRatio: false,
                    scales: {
                        y:  { position: 'left',  title: { display: true, text: 'Reviews', font: { size: 11 } }, grid: { color: '#f1f5f9' }, ticks: { font: { size: 11 } } },
                        y1: { position: 'right', title: { display: true, text: 'Avg ★',   font: { size: 11 } }, min: 0, max: 5, grid: { display: false }, ticks: { font: { size: 11 } } },
        ...
        // 4. Top Rated Spots (inside leaderboard lists)
    ...
    function renderTopList(tab, selector, items, metric) {
        ...
        el.innerHTML = items.map((s, i) => `
            <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f1f5f9;cursor:pointer;" onclick="window.openFeedbackSpotDetail(${s.id})">
                <span style="font-size:11px;font-weight:800;color:#94a3b8;width:18px;text-align:center;">${i + 1}</span>
                <div style="flex:1;min-width:0;">
                    <div style="font-size:13px;font-weight:600;color:#073B6B;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escHtml(s.name)}</div>
                    <div style="font-size:11px;color:#94a3b8;">${escHtml(s.municipality || '')}</div>
                </div>
                <span style="font-size:13px;font-weight:700;color:#0B5394;flex-shrink:0;">
                    ${metric === 'avg_rating'
                        ? `⭐ ${s.avg_rating.toFixed(2)}`
                        : `${(s.total_reviews).toLocaleString()} <span style="font-weight:400;color:#94a3b8;font-size:11px;">reviews</span>`}
                </span>
            </div>`).join('');
    }
    ...
    function getGalleryFilters() {
        const t  = document.getElementById('fb-search-input');
        const m  = document.getElementById('fb-filter-municipality');
        const c  = document.getElementById('fb-filter-category');
        const r  = document.getElementById('fb-filter-rating');
        const s  = document.getElementById('fb-sort-select');
        return {
            search:       t?.value.trim() || '',
            municipality: m?.value || '',
            category:     c?.value || '',
            min_rating:   r?.value || '',
            sort:         s?.value || 'most_reviewed',
        };
    }
    ...
    function renderGallery(grid, d) {
        ...
        grid.innerHTML = d.data.map(spot => {
            ...
            const imgTag = img
                ? `<img src="${escHtml(img)}" alt="${escHtml(spot.name)}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=\\'fb-spot-img-placeholder\\'><i class=\\'fas fa-mountain-sun\\'></i></div>';">`
                : `<div class="fb-spot-img-placeholder"><i class="fas fa-mountain-sun"></i></div>`;
            return `
                <div class="fb-spot-card" onclick="window.openFeedbackSpotDetail(${spot.id})" role="button" tabindex="0" aria-label="View feedback for ${escHtml(spot.name)}">
                    <div class="fb-spot-img-wrap">
                        ${imgTag}
                        <span class="fb-spot-category-badge">${escHtml(spot.category || 'Spot')}</span>
                    </div>
                    <div class="fb-spot-card-body">
                        <h3 class="fb-spot-name">${escHtml(spot.name)}</h3>
                        <div class="fb-spot-municipality"><i class="fas fa-location-dot"></i> ${escHtml(spot.municipality || 'Unknown')}</div>
                        <div class="fb-spot-rating-row">
                            <div>
                                ${renderStars(spot.avg_rating)}
                                <span class="fb-avg-rating" style="margin-left:6px;">${(spot.avg_rating || 0).toFixed(1)}</span>
                            </div>
                            <span class="fb-review-count">${(spot.total_reviews || 0).toLocaleString()} reviews</span>
                        </div>
                    </div>
                    <div class="fb-spot-card-footer">
    ...
    function getTableFilters() {
        ...
        const r   = document.getElementById('fb-filter-rating');
        const s   = document.getElementById('fb-sort-select');
        const df  = document.getElementById('fb-date-from');
        const dt  = document.getElementById('fb-date-to');
        return {
            search:       t?.value.trim() || '',
            municipality: m?.value || '',
            category:     c?.value || '',
    ...
    function renderTable(tbody, d) {
        ...
        tbody.innerHTML = d.data.map(row => {
            const av = `<div style="display:flex;align-items:center;gap:6px;">${avatarHtml(row.user_name, row.user_avatar, 28)}<span>${escHtml(row.user_name)}</span></div>`;
            return `
                <tr>
                    <td><span class="fb-table-spot-name" title="${escHtml(row.tourist_spot)}">${escHtml(row.tourist_spot)}</span></td>
                    <td><span style="font-size:12px;">${escHtml(row.municipality)}</span></td>
                    <td>${ratingBadge(row.rating)}</td>
                    <td><span class="fb-table-comment" title="${escHtml(row.comment || '')}">${escHtml(row.comment || '—')}</span></td>
                    <td>${av}</td>
                    <td style="font-size:12px;color:#64748b;white-space:nowrap;">${escHtml(row.date)}</td>
                    <td>
                        <button class="fb-page-btn" onclick="window.openFeedbackSpotDetail(null, ${row.id})" style="font-size:11px;padding:4px 10px;height:28px;" title="View spot feedback">
    ...
    //  SPOT DETAIL MODAL
    // ══════════════════════════════════════════════════════════════════════════
    async function openSpotDetail(spotId, feedbackId = null) {
        detailSpotId = spotId;
        detailPage   = 1;
        detailSort   = 'newest';
        ...
        if (!overlay) {
            ...
            overlay.innerHTML = `<div class="fb-modal" id="fb-modal-inner" role="dialog" aria-modal="true">
                ...
                <div class="fb-modal-body" id="fb-modal-body">
                    <div class="fb-empty-state"><i class="fas fa-spinner fa-spin"></i><p>Loading…</p></div>
                </div>
            </div>`;
        ...
        if (!spotId) { window.closeFeedbackModal(); return; }
    ...
    function renderModal(d) {
        ...
        const breakdownHtml = [5,4,3,2,1].map(star => {
            ...
            return `<li class="fb-breakdown-item">
                <span class="fb-breakdown-label">${star}★</span>
                <div class="fb-breakdown-bar-wrap"><div class="fb-breakdown-bar" style="width:${pct}%;"></div></div>
                <span class="fb-breakdown-count">${count.toLocaleString()}</span>
            </li>`;
        }).join('');

        const modalBody = document.getElementById('fb-modal-body');
        modalBody.innerHTML = `
            <div class="fb-modal-spot-hero">
                ${sliderHtml}
                <div class="fb-modal-spot-info">
                    <h2 class="fb-modal-spot-name">${escHtml(spot.name)}</h2>
                    <div class="fb-modal-spot-meta">
                        <span class="fb-meta-badge muni"><i class="fas fa-location-dot"></i> ${escHtml(spot.municipality || 'Unknown')}</span>
                        <span class="fb-meta-badge cat"><i class="fas fa-tag"></i> ${escHtml(spot.category || 'Spot')}</span>
                    </div>
                    <div class="fb-modal-big-rating">
                        <span class="fb-modal-big-num">${(spot.avg_rating || 0).toFixed(1)}</span>
                        <div>
                            <div class="fb-modal-big-stars">
                                ${[1,2,3,4,5].map(i => `<i class="fas fa-star fb-modal-big-star${i > Math.round(spot.avg_rating || 0) ? ' empty' : ''}"></i>`).join('')}
                            </div>
                            <div class="fb-modal-review-count">${(spot.total_reviews || 0).toLocaleString()} reviews</div>
                        </div>
                    </div>
                    <!-- Rating breakdown -->
            ...
            <div class="fb-reviews-header">
                ...
                <select id="fb-modal-sort" class="fb-filter-select" style="min-width:130px;font-size:12px;padding:6px 10px;">
            ...
            <div id="fb-modal-reviews-list">
                ${renderReviews(d.reviews)}
            </div>
            <div id="fb-modal-pagination" style="margin-top:12px;"></div>
        `;
    ...
    function renderReviews(reviews) {
        if (!reviews || !reviews.length) {
            return `<div class="fb-empty-state"><i class="fas fa-comment-slash"></i><h3>No Reviews</h3><p>This spot has no reviews yet.</p></div>`;
        }
        return reviews.map(r => {
            const imagesHtml = r.images && r.images.length
                ? `<div class="fb-review-images">
                    ${r.images.map(img => {
                        const url = imgUrl(img);
                        return `<img src="${escHtml(url)}" class="fb-review-img" onclick="window.openFeedbackLightbox('${escHtml(url)}')" alt="Review photo">`;
                    }).join('')}
                  </div>`
                : '';

            const tagsHtml = [
                r.crowd_level       ? `<span class="fb-review-tag crowd"><i class="fas fa-users"></i> ${r.crowd_level}</span>` : '',
                r.cleanliness       ? `<span class="fb-review-tag cleanliness"><i class="fas fa-broom"></i> ${r.cleanliness}</span>` : '',
                r.safety            ? `<span class="fb-review-tag safety"><i class="fas fa-shield-halved"></i> ${r.safety}</span>` : '',
            ].filter(Boolean).join('');

            return `
                <div class="fb-review-card">
                    <div class="fb-review-header">
                        <div class="fb-reviewer-info">
                            ${avatarHtml(r.user_name, r.user_avatar, 36)}
                            <div>
                                <div class="fb-reviewer-name">${escHtml(r.user_name)}</div>
                                <div class="fb-review-date">${escHtml(r.date)}</div>
                            </div>
                        </div>
                        <div class="fb-review-stars">
                            ${[1,2,3,4,5].map(i => `<i class="fas fa-star fb-review-star${i > (r.rating||0) ? ' empty' : ''}"></i>`).join('')}
                        </div>
                    </div>
                    ${r.comment ? `<p class="fb-review-comment">${escHtml(r.comment)}</p>` : ''}
                    ${tagsHtml ? `<div class="fb-review-tags">${tagsHtml}</div>` : ''}
                    ${imagesHtml}
                </div>`;
        }).join('');
    }
    ...
    function renderPagination(containerId, current, last, cb) {
        ...
        el.innerHTML = `
            <div class="fb-pagination">
                <span class="fb-pagination-info">Page ${current} of ${last}</span>
                <div class="fb-pagination-btns">
                    <button class="fb-page-btn" ${current <= 1 ? 'disabled' : ''} data-page="${current - 1}">
                        <i class="fas fa-chevron-left"></i>
                    </button>
                    ${pages.map(p => `<button class="fb-page-btn${p === current ? ' active' : ''}" data-page="${p}">${p}</button>`).join('')}
                    <button class="fb-page-btn" ${current >= last ? 'disabled' : ''} data-page="${current + 1}">
    ...
    function switchView(view) {
        currentView = view;
        const gallerySection = document.getElementById('fb-gallery-section');
        const tableSection   = document.getElementById('fb-table-section');
        const btnGallery     = document.getElementById('fb-btn-gallery');
        const btnTable       = document.getElementById('fb-btn-table');
        const dateFilters    = document.getElementById('fb-date-filters');

        if (gallerySection) gallerySection.style.display = view === 'gallery' ? '' : 'none';
        if (tableSection)   tableSection.style.display   = view === 'table'   ? '' : 'none';
        if (btnGallery) btnGallery.classList.toggle('active', view === 'gallery');
        if (btnTable)   btnTable.classList.toggle('active',   view === 'table');
        if (dateFilters) dateFilters.style.display = view === 'table' ? '' : 'none';

        if (view === 'gallery') loadGallery(1);
        else                    loadTable(1);
    }
    ...
    function openLightbox(url) {
        ...
        if (!lb) {
            ...
            lb.id        = 'fb-lightbox';
            lb.className = 'fb-lightbox-overlay';
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  GLOBAL EXPORTS (called from HTML onclick / SPA)
    // ══════════════════════════════════════════════════════════════════════════
    window.openFeedbackSpotDetail = openSpotDetail;
    window.closeFeedbackModal = function () {
        const o = document.getElementById('fb-modal-overlay');
    ...
    function init(force = false) {
        ...
        if (!grid) return; // Page not yet mounted

        // Wire up toolbar events (only once)
        if (!isInitialized) {
            const searchInput = tab.querySelector('#fb-search-input');
        ...
        // If content already rendered and not forced, perform silent background sync
        ...
        // Initial load with skeleton loaders
        Promise.all([
            loadDashboardStats(),
            loadGallery(1)
        ]);
    ...
    // SPA: listen for page show
    document.addEventListener('spa:page:shown', e => {
        if (e.detail?.page === 'feedback.php') init();
    });
    ...
    if (document.readyState === 'loading') {
        ...
        // Small timeout to allow SPA to inject the tab content first
        setTimeout(init, 50);
    }
})();
